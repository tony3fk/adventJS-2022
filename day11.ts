...
/* function getCompleted(part: string, total: string): string {
  ...
  const [elapsedHours, elapsedMinutes, elapsedSeconds]: number[] =
    part.split(':').map(Number);

  const [totalHours, totalMinutes, totalSeconds]: number[] =
    total.split(':').map(Number);

  const elapsedTotalSeconds: number =
    elapsedHours * 3600 +
    elapsedMinutes * 60 +
    elapsedSeconds;

  const totalTotalSeconds: number =
    totalHours * 3600 +
    totalMinutes * 60 +
    totalSeconds;
  ...
  const gcd: number = getGCD(elapsedTotalSeconds, totalTotalSeconds);

  const numerator: number = elapsedTotalSeconds / gcd;
  const denominator: number = totalTotalSeconds / gcd;
...
function getCompleted(part: string, total: string): string {
  const getSeconds = (time: number): number => time[0] * 3600 + time[1] * 60 + time[2] * 1;
  let numerator: number = getSeconds(part.split(":").map(Number));
  let denominator: number = getSeconds(total.split(":").map(Number));

  let divisor: number = numerator;
