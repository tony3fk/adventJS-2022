...
function distributeGifts(packOfGifts: Array<string>, reindeers: Array<string>) {
  const packWeight: number = packOfGifts.reduce((sum, gift) => sum + gift.length, 0);
  const weightCapacity: number = reindeers.reduce((sum, reindeer) => sum + (reindeer.length * 2), 0);
  return Math.floor(weightCapacity / packWeight);
}
